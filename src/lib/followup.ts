...
const makeCandidate = (
  ...
  channel: (["whatsapp", "telegram", "phone", "website", "instagram", "email"] as const).find((channel) => channel === lead.source) ?? "other",
  direction: lead.interests?.find((interest) => interest.isPrimary)?.direction ?? lead.classification.direction,
  reason,
  stage: lead.stage,
